...
import { Loader2, ArrowRight, ArrowLeft, CheckCircle2, Settings, LogOut } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (!data.hasSubscription || !data.hasCreditsRemaining) {
        router.push("/pricing");
        return;
      }
  ...
  const handleSubmit = async () => {
    if (!canSubmit || !user || !subscription) return;

    // Check if user has enough credits
    if (totalCombinations > subscription.videosRemaining) {
      setError(`Not enough credits. You need ${totalCombinations} but only have ${subscription.videosRemaining} remaining.`);
      return;
    }
          ...
          <div className="flex items-center gap-4">
            {subscription && (
              <Badge className="bg-green-500/10 text-green-500 border-green-500/20">
                {subscription.videosRemaining} credits
              </Badge>
            )}
            <Link href="/settings">
